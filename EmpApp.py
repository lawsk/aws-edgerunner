...
def applyLeave():
    ...
    start_date = datetime.strptime(StartLeave,'%Y-%m-%d').date()
    end_date = datetime.strptime(EndLeave,'%Y-%m-%d').date()
...
def manageLeave():
    ...
    # Update statement
    update_stmt = "UPDATE leaves SET start_date = (%(start_date)s), end_date = (%(end_date)s) WHERE emp_id = %(emp_id)s AND leave_status = 1"
